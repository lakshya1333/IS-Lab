...
def load_client_state():
    if not os.path.exists(CLIENT_STATE_FILE):
        return {"doctor_id": None, "ecc": {}, "server_keys": {}}
...
def fetch_server_keys(state):
    ...
    if resp.get("status") == "ok":
        ...
        print("Server keys fetched successfully.")
        return True
    else:
...
def register_doctor_client(state):
    ...
    print("Generating ECC keypair (for digital signatures)...")
    ecc_key = ECC.generate(curve="P-256")
    pub_key_pem = ecc_key.public_key().export_key(format="PEM")
    priv_key_pem = ecc_key.export_key(format="PEM")

    state["doctor_id"] = doctor_id
    state["ecc"] = {"public_key_pem": pub_key_pem, "private_key_pem": priv_key_pem}
...
def ecc_sign_message(state, message_bytes):
    """Sign a message with ECC private key."""
    priv_key = ECC.import_key(state["ecc"]["private_key_pem"])
...
def submit_report(state):
    ...
    signature_b64 = ecc_sign_message(state, msg_to_sign)
...
def homo_rsa_encrypt_amount(state, amount):
    """Encrypt amount using homomorphic RSA (exponent trick)."""
    if amount < 0 or amount > 100000:
        print("Amount must be between 0-100000.")
        return None
...
def doctor_menu(state):
    while True:
        ...
        print("3. Submit report (encrypted + signed)")
        ...
        if ch == "1":
            ...
            print(f"Current doctor ID: {doc_id or 'Not registered'}")
        elif ch == "0":
            break
        else:
