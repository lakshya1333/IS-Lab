...
# removed: from Crypto.Hash import MD5
...
def elgamal_sign(eg_private, msg_bytes):
    """Sign message with ElGamal using SHA-256 as the hash function."""
    p = int(eg_private["p"])
    g = int(eg_private["g"])
    x = int(eg_private["x"])

    # Use SHA-256 instead of MD5
    H = int.from_bytes(hashlib.sha256(msg_bytes).digest(), "big") % (p - 1)
    while True:
...
def submit_report(state):
    ...
    body = {
        ...
        "filename": filename,
        "timestamp": timestamp,
        # changed key name to reflect SHA-256
